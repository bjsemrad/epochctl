...
```bash
epochctl wallpaper toggle             # the full-screen picker
epochctl wallpaper list               # every image found, current marked with *
epochctl wallpaper set <path>
epochctl wallpaper next               # cycle without opening anything
epochctl wallpaper refresh            # after adding images
```

Applying goes through the shell rather than straight to hyprpaper, because the choice has to be
remembered somewhere: hyprpaper's config is generated by home-manager and read-only, and its
`listactive` does not follow a live switch, so nothing on that side can answer "what is set".
